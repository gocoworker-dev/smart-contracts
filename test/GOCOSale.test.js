...
const GOCOSale = artifacts.require('GOCOSale');
...
contract('GOCOSale', function ([_, owner, founderAccount, tokenSaleAccount, rewardAccount, investor1, investor2, investor3, anyone]) {
  ...
  beforeEach(async function () {
    ...
    this.crowdsale = await GOCOSale.new(this.openingTime, tokenSaleAccount, rewardAccount, NUMBER_OF_PERIOD, this.token.address, { from });
  ...
  context('When the sale is open with 3 investors purchasing tokens on first period', function () {
    ...
    it('should reject claim when period is finished since less than one minute', async function () {
      await this.crowdsale.buyTokens(investor1, { value: investmentAmount, from: investor1 });
      await time.increaseTo(this.openingTime.add(time.duration.hours(21)).add(time.duration.seconds(30)));
